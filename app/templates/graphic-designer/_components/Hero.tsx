...
export default function Hero() {
  ...
  // Subtle parallax + fade on the video as you scroll
  useEffect(() => {
    let ticking = false;
    const onScroll = () => {
      if (!ticking) {
        requestAnimationFrame(() => {
          ...
          el.style.transform = `scale(1.05) translateY(${pct * 12}%)`;
          el.style.opacity = `${1 - pct * 0.7}`;
          ticking = false;
        });
        ticking = true;
      }
    ...
    <section id="hero" ref={sectionRef} className={styles.hero}>
      ...
      <div className={styles.heroVideoBg}>
        <video
          ...
          aria-hidden="true"
        >
          <source src="/designer-showreel.mp4" type="video/mp4" />
        </video>
