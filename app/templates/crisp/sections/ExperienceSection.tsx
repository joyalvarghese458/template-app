...
import type { CrispExperience } from "../types";
  ...
  return (
    <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_18px_45px_rgba(15,23,42,0.06)] sm:p-8">
      <SectionHeading
        eyebrow="02"
        title="Experience"
        description="The primary resume section, organized for recruiter-friendly scanning and timeline clarity."
      />
      <div className="mt-6 space-y-6">
        {experiences.map((item, index) => (
          <article
            key={`${item.jobTitle}-${item.company}`}
            className="relative pl-6 sm:pl-8"
          >
            <span className="absolute left-0 top-2 h-3 w-3 rounded-full border-2 border-slate-300 bg-slate-900" />
            {index < experiences.length - 1 ? (
              <span className="absolute left-[5px] top-5 h-[calc(100%+18px)] w-px bg-slate-200 sm:left-[6px]" />
            ) : null}
