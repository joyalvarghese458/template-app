...
export default function SidebarSection(props: SidebarSectionProps) {
  ...
  return (
    <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_14px_36px_rgba(15,23,42,0.05)] sm:p-8 lg:p-6">
      <SectionHeading
      ...
      <div className="mt-5 space-y-3">
        {props.kind === "skills"
          ? props.items.map((group) => (
              <article key={group.category} className="rounded-[22px] bg-slate-50 p-4">
        ...
        {props.kind === "certifications"
          ? props.items.map((item) => (
              <article key={`${item.name}-${item.issuer}`} className="rounded-[22px] bg-slate-50 p-4">
        ...
        {props.kind === "languages"
          ? props.items.map((item) => (
              <article
                key={item.name}
                className="flex items-center justify-between gap-3 rounded-[22px] bg-slate-50 px-4 py-3"
              >
                <h3 className="text-sm font-semibold text-slate-950">{item.name}</h3>
        ...
        {props.kind === "achievements"
          ? props.items.map((item) => (
              <article key={`${item.title}-${item.date}`} className="rounded-[22px] bg-slate-50 p-4">
                <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between lg:flex-col lg:items-start">
        ...
        {props.kind === "references"
          ? props.items.map((item) => (
              <article key={`${item.name}-${item.contact}`} className="rounded-[22px] bg-slate-50 p-4">
                <h3 className="text-sm font-semibold text-slate-950">{item.name}</h3>
